...
- Python deps: `services/deps_installer.py` parses Python `import`/`from` statements via `ast`, maps import names to pip package names (`IMPORT_TO_PACKAGE`: sklearn→scikit-learn, yaml→pyyaml, PIL→Pillow, cv2→opencv-python, bs4→beautifulsoup4), and checks whether each required package is installed in the configured Python environment. **It no longer auto-installs missing packages.** If any dependency is missing, `/api/execute/python` and `/api/execute/tasks/{id}/test` return HTTP 400 with a clear message, and `task_runner` marks the `TaskRun` as failed with the missing package list in `error_message`. Users must install packages manually (e.g. via `uv pip install --python <path> <pkg>`) and can view the current environment's installed packages on the system settings page (`GET /api/settings/packages`). Python 解释器路径由用户在“系统设置”中配置（`Setting.python_executable_path`），未配置时开发模式回退到 `sys.executable`，打包版必须配置。
...
- Electron 后端进程管理：`ghost-flow-backend.exe`（PyInstaller onefile）运行时是父子双进程（引导器 + Python 子进程），终止时必须按进程树杀（`taskkill /pid <pid> /T /F`，见 `main.ts` `killProcessTree`），只 kill 父进程会残留子进程占用 17892 端口，导致下次启动新后端绑定失败（退出码 3）；`startBackend` 启动前也会先清理残留后端进程。
