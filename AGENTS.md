...
- Auto deps: `services/deps_installer.py` parses Python `import`/`from` statements via `ast`, maps import names to pip package names (`IMPORT_TO_PACKAGE`: sklearn→scikit-learn, yaml→pyyaml, PIL→Pillow, cv2→opencv-python, bs4→beautifulsoup4), installs missing packages via uv. **安装失败抛 `RuntimeError`（含 stderr 摘要）**：`/api/execute/python` 转为 400，`/api/execute/tasks/{id}/test` 转为 400，`task_runner` 写入 `TaskRun.error_message`。Python 解释器路径由用户在“系统设置”中配置（`Setting.python_executable_path`），未配置时开发模式回退到 `sys.executable`，打包版必须配置；uv 在开发模式从 PATH 查找，打包版通过 `GHOST_FLOW_RESOURCES_DIR` 环境变量定位随包分发的 `uv.exe`。
